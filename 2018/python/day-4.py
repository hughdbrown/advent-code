d = """[1518-03-18 00:03] Guard #3529 begins shift
...
data = sorted(d.splitlines())

start = re.compile(r"""^.*?Guard #(?P<n>\d+) begins shift$""")
asleep = re.compile(r"""^\[.*?\s00:(?P<min>\d+)\]\sfalls asleep$""")
wakes = re.compile(r"""^\[.*?\s00:(?P<min>\d+)\]\swakes up$""")
...
for line in data:
    line = line.strip()
    print(line)
    s, a, w = (x.match(line) for x in (start, asleep, wakes))
    if s:
        guard = int(s.groupdict()["n"])
    elif a:
        s_min = int(a.groupdict()["min"])
    elif w:
        w_min = int(w.groupdict()["min"])
        print(s_min, w_min, guard)
        c = g[guard]
        for i in range(s_min, w_min):
            c[i] += 1
        s_min, w_min = None, None
    else:
        assert False

guard, x = max(g.items(), key=lambda y: sum(y[1].values()))
most_asleep_minute, count = max(x.items(), key=lambda y: y[1])
print(guard * most_asleep_minute)

guard, x = max(g.items(), key=lambda y: max(y[1].values()))
most_asleep_minute, count = max(x.items(), key=lambda y: y[1])
print(guard * most_asleep_minute)
