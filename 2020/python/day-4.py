...
def passports(data):
    ...
    indexes = [i for i, line in enumerate(lines) if not line]
    if indexes[-1] != len(lines):
        indexes.append(len(lines))
    i = 0
    for index in indexes:
        d = {}
        for p in lines[i:index]:
            for x in p.split():
                k, v = x.split(':')
                d[k] = v
        yield d
...
def puzzle1(data):
    ...
    for passport in passports(data):
        # print(passport)
        count += all(key in passport for key in keys)
    return count
...
def puzzle2(data):
    ...
    for passport in passports(data):
        if all(key in passport for key in keys):
            count += (
                (len(passport['byr']) == 4 and 1920 <= int(passport['byr']) <= 2002) and
                (len(passport['iyr']) == 4 and 2010 <= int(passport['iyr']) <= 2020) and
                (len(passport['eyr']) == 4 and 2020 <= int(passport['eyr']) <= 2030) and
                (
                    150 <= int(passport['hgt'].strip('cm')) <= 193 if passport['hgt'].endswith('cm') else
                    59 <= int(passport['hgt'].strip('in')) <= 76 if passport['hgt'].endswith('in') else
                    False
                ) and
                ...
                (len(passport['pid']) == 9 and all(c in "0123456789" for c in passport['pid']))
            )
    return count
