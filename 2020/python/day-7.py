...
bag_no_re = re.compile(r"""
    ^
    (?P<container>.*?)
    bags\scontain\sno\sother\sbags.
""", re.VERBOSE)

data = """drab plum bags contain 5 clear turquoise bags, 5 striped aqua bags, 4 dotted gold bags, 4 plaid chartreuse bags.
...
def bags(data):
    ...
    for line in data.splitlines():
        m = bag_re.match(line) or bag_no_re.match(line)
        if m:
            g = m.groupdict()
            c = g['container']
            d[c] = {
                n[1]: int(n[0])
                for n in re.findall(r'(\d+)\s(.*?)\sbags?', line)
            }
    return d
