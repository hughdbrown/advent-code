...
def puzzle1(data):
    ...
    for line in data.splitlines():
        ...
        rr, c = rule.split(' ')
        lower, upper = int(rr.split('-')[0]), int(rr.split('-')[1])
        count += int(lower <= password.count(c) <= upper)
    return count
...
def puzzle2(ds):
    ...
    for line in data.splitlines():
        ...
        rr, c = rule.split(' ')
        lower, upper = int(rr.split('-')[0]) - 1, int(rr.split('-')[1]) - 1
        count += int((password[lower] == c) ^ (password[upper] == c)) 
    return count
