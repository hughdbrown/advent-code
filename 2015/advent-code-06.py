...
def parse(m):
    return tuple(int(m.groupdict()[a]) for a in ['start_x', 'start_y', 'end_x', 'end_y'])
...
def main1():
    ...
    def turn_off_f(curr):
        return 0
    def toggle_f(curr):
        return int(not bool(curr))
    array = np.zeros((1000, 1000), dtype=int)
    d = {
        "turn on": np.vectorize(turn_on_f, otypes=[np.int]),
        "turn of": np.vectorize(turn_off_f, otypes=[np.int]),
        "toggle ": np.vectorize(toggle_f, otypes=[np.int]),
    }
    for line in DATA:
        start_x, start_y, end_x, end_y = parse(toggle.match(line))
        fn = d[line[:7]]
        array[start_y:end_y + 1, start_x:end_x + 1] = fn(array[start_y:end_y + 1, start_x:end_x + 1])
    return np.sum(array)
